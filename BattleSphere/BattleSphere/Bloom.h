...
private:
	...
	ID3D11Texture2D* m_gaussTex[2] = { nullptr };

	ID3D11RenderTargetView* m_lowResRTV = nullptr;
	ID3D11RenderTargetView* m_fullResRTV[2] = { nullptr };

	ID3D11ShaderResourceView* m_gaussSRV[2] = { nullptr };				// SRV and UAV in middle
	ID3D11ShaderResourceView* m_bloomSRV[3] = { nullptr };				// Scene and bloom in low 

	ID3D11UnorderedAccessView* m_gaussUAV[2] = { nullptr };			// SRV and UAV in middle
