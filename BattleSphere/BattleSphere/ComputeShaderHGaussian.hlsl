...
static const int kernelHS = 49;
static const int kernelSize = 99;

groupshared float4 horizontalPoints[size_x + kernelSize - 1];
...
void CS_main(CS_input input)
{
	...
	horizontalPoints[input.GroupThreadID.x + kernelHS] = data;
	if (input.GroupThreadID.x == 0)
	{
		for (int i = 0; i < kernelHS; i++)
		{
			horizontalPoints[i] = InputTex.Load(float3(input.DispatchThreadID.x - kernelHS + i, input.DispatchThreadID.y, input.DispatchThreadID.z));
			horizontalPoints[size_x + kernelHS + i] = InputTex.Load(float3(input.DispatchThreadID.x + size_x + i, input.DispatchThreadID.y, input.DispatchThreadID.z));
		}
	}
	...
	for (int i = 0; i < kernelSize; i++)
		colour += horizontalPoints[i + input.GroupThreadID.x] * kernel[i];
