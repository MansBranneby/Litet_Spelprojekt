...
bool UserInterface::updateCountDown(float dt)
{
	...
	else if (m_countDownTimer >= 4.0f)
	{
		...
		{
			Sound::getInstance()->play(soundUI::e_traverse, 0.3f, -0.8f);
			Sound::getInstance()->play(soundUI::e_traverse, 0.25f, 0.0f);
			Sound::getInstance()->play(soundUI::e_back, 0.1f, 1.0f);
	...
	else
	{
		...
		if (m_countDownTimer > 1.0f && m_countDownTimer < 2.0f)
		{
			if (!m_countDownElements[1]->isDrawn())
			{
				...
				Sound::getInstance()->play(soundUI::e_countdown, 0.12f);
			}
